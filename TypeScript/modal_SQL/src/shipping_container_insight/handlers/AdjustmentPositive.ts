...
export class AdjustmentPositive {
	...
	public async initializeProperties() {
		try {
			this.adjustmentPositiveForm = document.querySelector(`#${idAdjustmentPositiveForm}`);
	...
	private setEventCopyToClipBoard() {
		// Event to copy
		const btnCopy = document.querySelector(`#${idBtnCopyAjsPositive}`);
